...
class ServiceProviderView(APIView):
    ...
    def get(self, request):
        ...
        y = ExtraInfo.objects.select_related('user', 'department').filter(user=current_user).first()
        try:
            service_provider = ServiceProvider.objects.select_related('sup_id').get(sup_id=y)
...
class ForwardCompaintView(APIView):
    ...
    def post(self, request, comp_id1):
        ...
        service_provider_details = ExtraInfo.objects.get(id=service_provider.sup_id.id)
...
class GenerateReportView(APIView):
    ...
    def get(self, request):
        """
        Generates a report of complaints for the caretaker's area, service_provider's area, service provider type, or service authority type.
        """
        user = request.user

        is_caretaker = hasattr(user, 'caretaker')
        is_service_provider = hasattr(user, 'service_provider')
        is_admin = hasattr(user, 'admin')
        is_service_provider = hasattr(user, 'service_provider')
        is_service_authority = hasattr(user, 'service_authority')

        # Check if user is a service_provider
        try:
            service_provider = ServiceProvider.objects.get(sup_id=user.extrainfo)
            is_service_provider = True
        except ServiceProvider.DoesNotExist:
            is_service_provider = False
        
        try:
            is_service_authority = ServiceAuthority.objects.get(sup_id=user.extrainfo)
